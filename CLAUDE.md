...
internal/router/                  Unified OpenAI-compatible router
  ...
  router_socket.go                   router.sock (C9, launched mode): peer-identity admission + the
                                      socket mux (the TCP mux; /v1/messages needs a client credential
                                      to reach the real Anthropic API)
...
- Startup validation (`warnAnthropicModelMap`, `internal/app/app.go`) warns about a `modelMap` key colliding with an existing managed alias or endpoint-prefixed id (it would silently shadow that route, since the map is resolved before every other dispatch check) and a target that resolves to nothing configured. There is no bind or TLS requirement for `router.anthropic` / `router.passthrough`: like every other router route, exposure is controlled by `--router-bind`, and TLS is opt-in via `--router-tls-cert`.

**Credential passthrough** (`router.passthrough` config, `internal/router/router_passthrough.go`). Each entry mounts `/<name>/` on the router and forwards everything under it byte-for-byte to `upstream`: headers (the client's own `Authorization` included), body, SSE, and WebSocket upgrades. `/chatgpt/codex/responses` becomes `<upstream>/codex/responses`. relayLLM stores no credential, the same model as the Anthropic passthrough. It exists for OpenAI's two surfaces: ChatGPT-subscription OAuth clients (Oh My Pi's `openai-codex` provider, the Codex CLI) talk to `https://chatgpt.com/backend-api`, and API-key clients talk to `https://api.openai.com`. OMP points at it with `providers.openai-codex.baseUrl: http://127.0.0.1:8180/chatgpt` in `~/.omp/agent/models.yml`; OMP officially supports a streaming-proxy base URL and keeps its `/usage` calls direct to chatgpt.com. Routing is by path prefix, never by the body's `model` field: OpenAI's routes are the same paths the router serves for local models, and model dispatch would send a mistyped local model name, prompt included, to a cloud provider. There is no `modelMap`, since OMP and Codex already reach local models through their own provider config. Invalid entries (reserved names `v1`/`api`/`models`/`health`, a non-http(s) upstream, plain `http` to a non-loopback host) are logged and skipped. A WebSocket held open between turns reports `idle` on the dashboard, never `stalled`, and its frames are metered through `meteredResponseWriter.Hijack`. Under relay these routes are also served on router.sock (below): relay's model endpoint forwards a client's provider request here with the client's own credential still on it.

**router.sock** (`internal/router/router_socket.go`, `internal/relay/launch.go`, `internal/relay/bridge_client.go`, `internal/app/model_host.go` — launched mode only). When relay launches relayLLM, `Hello` (`internal/relay/launch.go`) captures relay's own identity off that same connection: on the *connecting* end of a Unix stream socket, `LOCAL_PEERTOKEN` names the *accepting* process, not the caller's own — confirmed empirically rather than assumed, since it is the mirror image of the well-known accepting-side use for `LOCAL_PEERCRED`/Hello and isn't obvious without checking. Hello cross-checks that token's pid against the `Hello` OK reply's `relay_pid` and stores the resulting `(pid, pidversion)` (`relay.RelayIdentity()`); any failure to capture or a pid mismatch fails Hello closed, exactly like every other launch failure. `--router-socket` (default `{data-dir}/router.sock`, mode 0600; `removeStaleSocket` refuses to unlink anything at that path that isn't already a socket, so a misconfigured path or a stray regular file is a startup error, not something silently overwritten) then serves the SAME router on a Unix socket that admits only that one peer — every accepted connection's kernel audit token is checked against `RelayIdentity()` once, at accept (`RelayRouter.ListenSocket`'s `ConnContext`), not per request, comparing the FULL `(pid, pidversion)` pair — and refuses everyone else with `401 {"error":{"message":"unauthorized","type":"authentication_error"}}`. `internal/app/app.go`'s `Main` builds exactly one `router.RelayRouter` via `router.BuildRelayRouter` and reuses it for both decisions: `RelayRouter.MaybeServeTCP` binds `--router-port` if there's anything to serve, and when launched the SAME object is kept (never rebuilt) for `ListenSocket` even if `MaybeServeTCP` reported nothing to serve over TCP — router.sock must exist whenever relay launched this process, independent of whether the TCP listener is also configured. Building only once matters beyond tidiness: `setAnthropic`/`setPassthrough` (called from `BuildRelayRouter`) each log a warning per invalid config entry, so an earlier shape that rebuilt a second router whenever the first wasn't bound for TCP doubled every one of those lines. `RelayRouter.SocketHandler()` builds the socket's mux: `p.mux` itself (so `/api/`, every configured `router.passthrough` `/<name>/` route and anything `setPassthrough` mounts later are all served, with no second registration to drift), with only `/v1/messages` and `/v1/messages/count_tokens` overridden. Those routes forward the CLIENT'S OWN credential byte-for-byte, exactly as on TCP: relay's model endpoint (`plans/client-model-routing.md`) classifies each request, serves a model in its catalog locally, and sends a provider's own request here with the client's `Authorization`/`x-api-key` untouched and its own credentials (`X-Relay-Key`, every `x-relay-*`) stripped. Relay holds no upstream credential and never forwards one; relayLLM still stores none. The one narrowing that remains is a belt-and-braces guard on the two Anthropic routes: a model outside `router.anthropic.modelMap` is forwarded to the real Anthropic API only when the request carries `Authorization` or `x-api-key` (`hasClientCredential`). Relay strips both from a request it served locally, so a local-model request that reached this mux by mistake (a stale relay catalog) 404s (Anthropic-shaped `not_found_error`) rather than sending its prompt to api.anthropic.com credential-less. `/v1/messages/count_tokens` keeps its OWN socket handler (`handleAnthropicCountTokensSocket`): a mapped model answers with the same byte-based estimate `handleAnthropicCountTokens` uses and must never share `/v1/messages`'s redirect handler, which would silently turn a token-count probe into a real upstream generation call; an unmapped one is the real Anthropic counter under the same credential rule. Every proxied response — direct dispatch or the Anthropic-modelMap redirect alike — carries `X-Relay-Model-Target` (a bare managed alias, or `endpoint/upstreamID`; `router.go`'s `setModelTargetHeader` walks a chain of `Unwrap() http.ResponseWriter` wrappers so it reaches `translatingResponseWriter`'s real writer even through the Anthropic-redirect re-entrant dispatch, since that writer buffers headers separately and never copies them onto the real response on its own).

**Header stripping, on `--router-port` as well as router.sock**: `newUpstreamProxy` (the reverse proxy shared by every managed-server/endpoint/virtual-model dispatch AND `/v1/audio/transcriptions` — everything except the `/api/` Anthropic passthrough and a `router.passthrough` `/<name>/` route, which still forward the caller's real credential byte-for-byte on purpose) strips an inbound `X-Api-Key` header and any `X-Relay-*` header before the request reaches the real backend, and strips any `X-Relay-*` header an upstream response tries to add before `X-Relay-Model-Target` is set on the reply. An endpoint's upstream key comes only from `settings.json`'s `openai.endpoints[].apiKey`, set by `newUpstreamProxy` itself; a standalone `--router-port` client cannot pass one through a header.

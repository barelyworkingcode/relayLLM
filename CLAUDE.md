...
- Startup validation (`warnAnthropicModelMap`, main.go) warns about a `modelMap` key colliding with an existing managed alias or endpoint-prefixed id (it would silently shadow that route, since the map is resolved before every other dispatch check) and a target that resolves to nothing configured. There is no bind or TLS requirement for `router.anthropic` / `router.passthrough`: like every other router route, exposure is controlled by `--router-bind`, and TLS is opt-in via `--router-tls-cert`.

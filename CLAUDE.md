...
provider_pi_models.go     pi model discovery via `pi --list-models` (cached)
provider_pi_overlay.go    Per-project .pi/ overlay (models.json, settings.json, auth.json symlink)
provider_ollama.go        Ollama HTTP provider (NDJSON streaming)
provider_openai.go        OpenAI-compatible HTTP provider (SSE streaming)
provider_openai_config.go OpenAIEndpoint / virtual-LLM config structs, TLS pinning fields
...
provider_claude_history.go  Claude CLI history JSONL read/delete, including over SSH
...
relay_router.go           Unified OpenAI-compatible router: lifecycle, core dispatch, StartRelayRouter
relay_router_models.go    /v1/models catalog + virtual-row building
relay_router_virtual.go   Virtual-model target resolution/candidates + failover routing
relay_router_virtual_affinity.go  Per-conversation target pinning for virtual-model failover
relay_router_audio.go     /v1/audio/transcriptions multipart handling
relay_router_rewrite.go   reasoning_effort / chat_template_kwargs body rewrites
relay_router_anthropic.go        Anthropic Messages API compatibility: routes, passthrough, redirect dispatch
relay_router_anthropic_translate.go  Pure Anthropic<->OpenAI request/stream translation
relay_router_passthrough.go      router.passthrough: /<name>/ byte-for-byte proxy, client credential included (ChatGPT Codex, api.openai.com)
relay_router_endpoint_tls.go     TLS pinning (caFile/pinSHA256) for the relayLLM-to-endpoint hop
relay_router_proxy_registry.go   Reachability + model-list cache for configured OpenAI endpoints (15s TTL)
...
**Anthropic Messages API compatibility** (`router.anthropic` config, `relay_router_anthropic.go` / `relay_router_anthropic_translate.go`). Lets the `claude` CLI (Claude Code) point at the router via `ANTHROPIC_BASE_URL` and get real Anthropic models transparently proxied through to `api.anthropic.com`, plus an optional config-driven redirect of specific model ids to a local managed/virtual/endpoint model. Absent `router.anthropic` section → `/v1/messages`, `/v1/messages/count_tokens`, and the `/api/` passthrough prefix all 404 — zero behavior change, same off-by-default shape as `reasoningEffortMap` above.
...
Optionally also on TCP: `--http-port` / `RELAY_LLM_HTTP_PORT` (empty = disabled, the default) bound to `--http-bind` / `RELAY_LLM_HTTP_BIND` (default `127.0.0.1`; comma-separated to bind more than one interface, e.g. `127.0.0.1,192.168.64.1`), with `--http-tls-cert` / `--http-tls-key` optionally available for transport encryption. This is a *second front serving the same mux/route table* as the socket — session/terminal/permission/status routes are identical either way — but it is **not** behind `bearerAuth`: it's served anonymously, exactly like `--router-port`, protected only by which `--http-bind` addresses actually got bound. (This reverses an earlier version of this front that reused the socket's `bearerAuth` wrapping and required a token in the URL for `/status`; that was more restrictive than `--router-port` ever was for no good reason once the actual protection model is "control the bind address," so it was removed — see ROADMAP's Closed section.) It exists because the `/status` dashboard is a browser page and nothing else reaches it: relay's front door is a Unix socket only Eve dials, and Eve proxies only the specific `/api/*` + `/ws` routes its own code knows about. Each `--http-bind` address binds best-effort (`listenAll` in `main.go`): one that can't be bound (e.g. a gateway IP not locally assignable in this deployment) is logged and skipped rather than taking every other bind down with it; only a total failure — every configured address rejected — is fatal.
...
`auth.go::bearerAuth` validates every request against `--token` / `RELAY_LLM_TOKEN`, but only on the Unix socket (`--socket`). Empty token + standalone mode → auto-generated 64-char hex (not logged for security; set the env var to pin). Token comparison is constant-time via `crypto/subtle.ConstantTimeCompare`. The same token is plumbed through `SessionManager` → `ClaudeProvider` → hook child env as `RELAY_LLM_HOOK_TOKEN`; the hook binary uses it on its `/api/permission` POST. The one carrier accepted is `Authorization: Bearer <token>` — every caller of the socket is a machine client (relay's dispatcher, the permission hook, a direct socket client), never a browser, so there's no cookie or query-param fallback to bootstrap.

`--http-port`'s TCP front (see "Optionally also on TCP" above) is **not** wrapped in `bearerAuth` at all — it's anonymous, gated only by `--http-bind`, the same posture `--router-port` has always had. An `Authorization` header sent to it is simply never inspected.

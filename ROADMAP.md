...
|----------|------|-----------|-------|
| Low | `provider_claude.go` permission/hook integration tests | medium | Hook subprocess + permission flow (the *runtime* half — argv/env assembly is now hermetic, see ADR-008). Requires installable hook binary; defer until next time the hook path is touched. |
| Low | `config.go` unified-loader fallback chain (54%) | small | settings.json → legacy json → env. Table-test the precedence + `~` expansion; regression-prone "two sources, one loader" merge. |
...
- `mcp_client.go` real `MCPManager` 0% → ~88% hermetic. The subprocess `Start()` stays live-tier (`mcp_integration_test.go`); the rest — `CallTool` dispatch + result extraction, progress streaming, routing errors (unknown tool / disconnected server), `ChatToolDefs` shaping, accessors, `Close` reset — is now tested against a real in-process MCP server over the SDK's `NewInMemoryTransports`. Extracted `newMCPClient()` from `Start` so the test drives the *production* progress-notification handler (not a replica). Mutation-checked: dropping the per-call progress-sink registration fails the streaming test. De-flaked the progress test along the way — surfaced a real property (progress dispatch is async and the sink is deleted on `CallTool` return, so a late notification is dropped); the test now uses an in-process release barrier instead of assuming all progress lands before the result. `mcp_client_test.go`; no production behavior changed beyond the `newMCPClient` extraction.
- Pre-commit hook now runs the hermetic suite under the race detector (`go test -race ./...`). Measured cost: ~+1s steady-state (full hook ~2.5s→~2.9s warm), one-time ~+4s only when the instrumented build cache is cold (`go clean -cache` / Go upgrade). Caught what motivated it — the `TestWS_RenameSession_UpdatesName` race (fixed in the spawn-builder work below). Docs synced in `CLAUDE.md` + `.githooks/pre-commit` header.
- Hermetic regression coverage for provider spawn + the auth boundary. Extracted Claude/pi argv+env assembly out of `Start()` into pure builders (`buildClaudeArgs`/`buildClaudeEnv`/`effectivePermissionMode`, `buildPiArgs`/`resolveSkillDir`) so the security-sensitive flag matrix — the `bypassPermissions` / `--dangerously-skip-permissions` / `RELAY_LLM_HEADLESS` escape hatch, `--resume`, `--model`, policy tools, pi model routing + `--skill` auto-mount — is asserted on the default tier instead of only `//go:build live`/`llm`. New `provider_claude_spawn_test.go`, `provider_pi_spawn_test.go`, and a `security_regression_test.go` (relay's `TestSec_*` convention) covering the bearer-auth boundary (37.5%→87.5% on `auth.go`), headless isolation, and project-token fail-closed. Mutation-checked: leaking headless to a default session fails both the mechanical and security guards. Also fixed a pre-existing data race in `TestWS_RenameSession_UpdatesName` (test read `sess.Name` without the session lock that production `RenameSession` writes under). Full suite green under `-race`. See ADR-008. Tiers run: hermetic + `-race`; `live`/`llm` untouched (no provider runtime changed).

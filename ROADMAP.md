...
|----------|------|-----------|-------|
| Low | `provider_claude.go` permission/hook integration tests | medium | Hook subprocess + permission flow (the *runtime* half — argv/env assembly is already hermetic, see `buildClaudeArgs`'s doc comment). Requires the installable hook binary (`cmd/hook/`); defer until next time the hook path is touched. |
